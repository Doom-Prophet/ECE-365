...
class Question1(object):
    def kMeans(self, data, K, niter):
        ...
        **For grading purposes only:**
        ...
        centers = np.array(data[np.random.choice(data.shape[0], size=K, replace=False), :])
        for _ in range(niter):
            distance = dist.cdist(centers, data, 'euclidean')
            labels = np.argmin(distance, axis=0)
            # print([data[labels==i, :] for i in range(K)])
            # print([data[labels==i, :].mean(axis=0) for i in range(K)])
    ...
    def calculateJ(self, data):
        ...
        # Put your code below
        for K in range(2, 11):
            labels, centers = self.kMeans(data, K, niter=100)
            err[K-2] = sum([((data[labels==i]-centers[i])**2).sum() for i in range(K)])
            # for i in range(K):
            #     print(((data[labels==i]-centers[i])**2).shape)
            #     print(sum(((data[labels==i]-centers[i])**2)))
            #     print(((data[labels==i]-centers[i])**2).sum())
            # print("#################")
        return err
...
class Question2(object):
    def trainVQ(self, image, B, K):
        ...
        **For grading purposes only:**
        ...
        for i in range(N//B):
            for j in range(M//B):
                data.append(image[B*i:B*(i+1), B*j:B*(j+1)].flatten())
        ...
        **For grading purposes only:**
        ...
        for i in range(int(N/B)):
            for j in range(int(M/B)):
                block = image[B*i:B*(i+1),B*j:B*(j+1)].flatten()
                cmpimg[i, j] = classifier.predict(block.reshape(1, -1))
        return cmpimg
    ...
    def decompressImg(self, indices, codebook, B):
        ...
        """
        # Put your code below
        X, Y = indices.shape                # X=N//B; Y=M//B
        rctimage = np.zeros((X*B, Y*B))  
        for x in range(X):
            for y in range(Y):
                rctimage[B*x: B*(x+1), B*y: B*(y+1)] = codebook[int(indices[x][y])].reshape(B, B)
        return rctimage
        # return rctimage
...
class Question3(object):
    def generatePrototypes(self, traindata, trainlabels, K_list):
        ...
        **For grading purposes only:**
        ...
        for K in K_list:
            dataElement = np.zeros((K*len(classes), traindata.shape[1]))
            labelElement = np.zeros((K*len(classes), ))
            for i in range(len(classes)):
                classifier = KMeans(n_clusters=K, init='k-means++')
                classifier.fit(traindata[trainlabels==i])
                dataElement[i*K:(i+1)*K, : ] = classifier.cluster_centers_
                labelElement[i*K:(i+1)*K] = np.array([classes[i]]*K)
            proto_dat_list.append(dataElement)
            proto_lab_list.append(labelElement.astype(int))
...
class Question4(object):
    def benchmarkRSS(self, trainfeat, trainresp, valfeat, valresp):
        ...
        """
        # Put your code below
        predictResponse = np.zeros(valfeat.shape[0])
        rss = np.linalg.norm(valresp - predictResponse) ** 2 / valresp.shape[0]
        return rss
    ...
    def OLSRSS(self, trainfeat, trainresp, valfeat, valresp):
        ...
        predictResponse = LR.predict(valfeat)
        rss = np.linalg.norm(valresp - predictResponse) ** 2 / valresp.shape[0]
        return rss
    ...
    def RidgeRSS(self, trainfeat, trainresp, valfeat, valresp):
        ...
        for i in range(len(a)):
            LR = linear_model.Ridge(alpha=a[i])
            LR.fit(trainfeat, trainresp)
            predictResponse = LR.predict(valfeat)
            rss_array[i] = np.linalg.norm(valresp - predictResponse) ** 2 / valresp.shape[0]
        best_a = a[np.argmin(rss_array)]
        best_rss = rss_array[np.argmin(rss_array)]
        optimizedLR = linear_model.Ridge(alpha=best_a)
        optimizedLR.fit(trainfeat, trainresp)
        coef=optimizedLR.coef_
    ...
    def LassoRSS(self, trainfeat, trainresp, valfeat, valresp):
        ...
        # Put your code below
        for i in range(len(a)):
            LR = linear_model.Lasso(alpha=a[i])
            LR.fit(trainfeat, trainresp)
            predictResponse = LR.predict(valfeat)
            rss_array[i] = np.linalg.norm(valresp - predictResponse) ** 2 / valresp.shape[0]
        best_a = a[np.argmin(rss_array)]
        best_rss = rss_array[np.argmin(rss_array)]
        optimizedLR = linear_model.Lasso(alpha=best_a)
        optimizedLR.fit(trainfeat, trainresp)
        coef=optimizedLR.coef_
        return (rss_array, best_a, best_rss, coef)
